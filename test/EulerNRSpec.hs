...
import NState
import NSpell                 (applyNSpellWorld)
import Physics.EulerNR        (eulerNR)

-- simple two-body sanity check: after one Euler step the bodies
-- have moved toward each other.
spec :: Spec
spec = describe "Euler numeric integrator" $ do
  let c1 = AtomicC "1"
      ...
      end   = applyNSpellWorld spell dt start
      q1'   = lookupPos c1 end
      q2'   = lookupPos c2 end

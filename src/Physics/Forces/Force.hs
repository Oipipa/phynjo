...
instance Eq Force where
  ...
  Spring i1 j1 k1 l1   == Spring i2 j2 k2 l2   = i1 == i2 && j1 == j2 && k1 == k2 && l1 == l2
...
instance Show Force where
  ...
  show (Spring i j k l)  = "Spring "   ++ show i ++ " " ++ show j ++ " " ++ show k ++ " " ++ show l
  show (Drag d)          = "Drag "     ++ show d
  show (Custom _)        = "Custom <force>"

(<+>) :: Force -> Force -> Force
Custom f1 <+> Custom f2 = Custom $ \st c -> addV (f1 st c) (f2 st c)
_         <+> _         = error "(<+>): only Custom forces can be combined here"
infixr 6 <+>
